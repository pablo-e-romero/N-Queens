...
struct BoardBuilderTests {
    @Test func boardHasCorrectDimensions() {
        let board = BoardBuilder.make(from: GameState(boardSize: 4))
    ...
    @Test func queenIsReflectedInCell() {
        ...
        let board = BoardBuilder.make(from: GameState(boardSize: 4, placedQueens: [queen]))
    ...
    @Test func conflictIsReflectedInCell() {
        ...
        let board = BoardBuilder.make(from: GameState(boardSize: 4, conflictingPositions: [conflicting]))
    ...
    @Test func cellPositionsMatchCoordinates() {
        let board = BoardBuilder.make(from: GameState(boardSize: 3))
        for row in 0..<3 {
            for col in 0..<3 {
